...
SGX_FLAGS_RESERVED = (~(SGX_FLAGS_INITTED | SGX_FLAGS_DEBUG |
                        SGX_FLAGS_MODE64BIT | SGX_FLAGS_PROVISION_KEY | SGX_FLAGS_EINITOKEN_KEY))
...
class c_sgx_attributes(Structure):
    _fields_ = [("flags", c_ulonglong), ("xfrm", c_ulonglong)]
...
class c_sgx_misc_attribute(Structure):
    _fields_ = [("secs_attr", c_sgx_attributes), ("misc_select", c_uint)]
...
class sgx_misc_attribute:
    ...
    def __repr__(self):
        return "{{ secs_attr : {:x}, misc_select: {:x} }}".format(
            self._secs_attr,
            self._misc_select
        )
